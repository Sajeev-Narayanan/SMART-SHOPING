...
public class viewpreviousorders extends AppCompatActivity {
    ...
    String[] shop_name, product_name, quantity, date, image,prid;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_viewpreviousorders);
        viewpreviousorders=(ListView)findViewById(R.id.ls1);
        SharedPreferences sh= PreferenceManager.getDefaultSharedPreferences(getApplicationContext());
        String hu = sh.getString("ip", "");
        String url = "http://" + hu + ":5005/viewpreviousorder";
                        ...
                        try {
                            ...
                            if (jsonObj.getString("status").equalsIgnoreCase("ok")) {
                                ...
                                for(int i=0;i<js.length();i++)
                                {
                                    ...
                                    prid[i]=u.getString("product_id");



                                }

                                viewpreviousorders.setAdapter(new Custome_viewpreviousorders(getApplicationContext(),shop_name,product_name,quantity,date,image,prid));
